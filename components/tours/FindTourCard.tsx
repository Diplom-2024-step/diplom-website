...
const FindTourCard = () => {
  ...
  const [adults, setAdults] = useState<number>(adultsFromRouter ? parseInt(adultsFromRouter) : 1);
  const [children, setChildren] = useState<number>(childrenFromRouter ? parseInt(childrenFromRouter) : 0);

  const [toCityIdFromRoute, setToCityIdFromRoute] = useSearchParam("toCity");
  const [fromCityIdFromRoute, setFromCityIdFromRoute] = useSearchParam("fromCity");

  const [durationFromRouter, setDurationFromRouter] = useSearchParam("duration");
  ...
  const [fromCity, setFromCity] = useState<GetCityDto>(
{
  ...
  const endDate = addDays(currentDate, durationFromRouter ? parseInt(durationFromRouter) : 7);
